...
generate_random_walk <- function(num_steps = 90) {
  start_date <- as.POSIXct(as.integer(Sys.time()) * stats::runif(1), origin = "1970-01-01")

  increments <- stats::rnorm(num_steps)
  cumulative_sum <- cumsum(increments)
  time_series <- c(0, cumulative_sum) + stats::rnorm(1, 0, 50) + 25
...
random_plotly_box <- function(color, n = 50) {
  plotly::plot_ly(x = ~stats::rnorm(n), type = "box", color = I(color))
}

random_plotly_line <- function(color, n) {
  plotly::add_lines(
    plotly::plot_ly(generate_random_walk(n)),
    x = ~ date,
    y = ~ value,
    color = I(color),
    fill = "tozeroy",
