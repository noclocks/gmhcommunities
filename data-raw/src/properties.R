...
#'
#' @section Functions:
#' - `parse_properties_response()` - Parse the entire properties response
...
parse_properties_response <- function(resp) {
...
parse_property_base_data <- function(property_tbl_init) {
  ...
  select_cols <- base::intersect(
    expected_cols,
    colnames(property_tbl_init)
  )

  excluded_cols <- base::setdiff(
    expected_cols,
    colnames(property_tbl_init)
  )
...
parse_property_address_data <- function(property_tbl_init) {
  ...
  sub_addresses <- hold |>
    ...
    dplyr::rename(tidyselect::all_of(sub_addresses_rename_vec))
...
#'
#' @details
#' The response is parsed into a merged tibble containing data across the
#' following sub-tables:
#'
#' - `floor_plans` - main floor plan information (e.g. name, unit counts)
#' - `unit_types` - the unit types for the floor plans
#' - `rooms` - number of bedrooms and bathrooms
#' - `square_feet` - min, max, and average square feet
#' - `market_rent` - min, max, and average market rent
#' - `images` - image files associated with the floor plans
#'
...
parse_floorplans_response <- function(resp) {
...
# require(tibblify)
#
# api_spec <- tibblify::parse_openapi_spec(
#   "dev/sandbox/mitmproxy/entrata.apispec.yml"
# )
#
# properties_api_spec <- api_spec |>
#   dplyr::filter(endpoint == "/properties") |>
#   dplyr::pull(spec) |>
#   purrr::pluck(1)
#
#
# tspec <- tibblify::parse_openapi_schema(
#   "inst/extdata/entrata/properties/getProperties/getProperties.response.schema.json"
# )
#
# hold <- tibblify::tibblify(resp_properties, properties_api_spec)
#
# entrata_resp_validate <- function(resp, schema) {
#
#   check_response(resp)
#
...
#       c(
#         "Entrata API Response Validation Error detected for {.arg resp}:",
#         error_msg
#       )
#     )
#   }
...
#
#
# json_schema <- "inst/extdata/entrata/properties/getProperties/getProperties.response.schema.json"
#
# jsonvalidate::json_validate(
#   json = resp_properties,
#   schema = json_schema,
#   verbose = TRUE,
#   engine = "ajv"
# )
