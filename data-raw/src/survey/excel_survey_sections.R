...
# source("data-raw/R/read_market_survey_data.R")
#
# wb_path <- "data-raw/data/original/UNLOCKED - 1047 Commonwealth Market Survey Updated.xlsm.xlsx"
#
# # commonwealth
...
#   leasing_week = "2024-12-01"
# )
#
# # 1330 boylston
# boylston_market_survey_data <- read_raw_market_survey_data(
#   wb_path = wb_path,
#   sheet = "1330 Boylston",
#   property_id = "Boylston",
#   property_name = "1330 Boylston",
#   leasing_week = "2024-12-01"
# )
#
# # van ness
# van_ness_market_survey_data <- read_raw_market_survey_data(
#   wb_path = wb_path,
#   sheet = "Van Ness",
#   property_id = "vanness",
#   property_name = "Van Ness",
#   leasing_week = "2024-12-01"
# )
#
# # bower
# bower_market_survey_data <- read_raw_market_survey_data(
#   wb_path = wb_path,
#   sheet = "Bower",
#   property_id = "bower",
#   property_name = "Bower",
...
# )
#
# market_survey_data_by_property <- list(
#   commonwealth = commonwealth_market_survey_data,
#   boylston = boylston_market_survey_data,
#   van_ness = van_ness_market_survey_data,
#   bower = bower_market_survey_data
# )

cache_file <- "data-raw/cache/market_survey_data_by_property.qs"

market_survey_data_by_property <- qs2::qs_read(
  file = cache_file
)

# qs2::qs_save(
#   object = market_survey_data_by_property,
#   file = cache_file
# )

cli::cli_alert_success(
  c(
    "Initial Market survey data preparation successful.",
    "Data saved to: {.path {cache_file}}"
  )
)


# merge across properties -------------------------------------------------

property_summary_data <- market_survey_data_by_property$commonwealth$property_summary$property_summary_wide |>
  dplyr::bind_rows(market_survey_data_by_property$boylston$property_summary$property_summary_wide) |>
  dplyr::bind_rows(market_survey_data_by_property$bower$property_summary$property_summary_wide) |>
  dplyr::bind_rows(market_survey_data_by_property$van_ness$property_summary$property_summary_wide) |>
  janitor::clean_names() |>
...
readr::write_csv(
  ...
  "data-raw/data/working/market_survey/property_summary/property_summary_data.csv"
)
...
dplyr::glimpse(property_amenities_data_)
...
unit_amenities_data <- commonwealth_market_survey_data$amenities$unit_amenities_wide |>
  ...
  dplyr::mutate_at(
    dplyr::vars(private_bathrooms:tv_included_in_rent, furniture_included_in_rent),
    ~ ifelse(.x == "Yes", TRUE, FALSE)
  )
...
parking_data <- commonwealth_market_survey_data$parking$parking |>
  dplyr::bind_rows(boylston_market_survey_data$parking$parking) |>
  dplyr::bind_rows(bower_market_survey_data$parking$parking) |>
  dplyr::bind_rows(van_ness_market_survey_data$parking$parking) |>
  janitor::clean_names()
...
# View(average_rents_by_unit_type_data)
